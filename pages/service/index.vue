    ...
    <v-col cols="12">
      ...
      <v-card height="103" class="mx-10 mt-5 br-15" outlined>
        <v-row justify="space-between" class="pa-11">
          ...
          <span>
            <v-btn to="/service/add" icon>
              <img src="~/assets/img/PlusCircle.svg" alt="">
            </v-btn>
          </span>
        </v-row>
      </v-card>
      <v-card height="103" outlined class="ma-3 mx-10 br-15" v-for="service in services" :key="service.id">
...
export default {
  ...
  computed: {
    services() {
      return this.$store.getters['get_service']
    },
    pageLength(){
      return this.$store.getters['get_servicePageLength']
    },
  ...
  methods: {
    ...
    deleteCategory(id) {
      ...
      this.$store.commit('public/set_statusDelete', 'service')
